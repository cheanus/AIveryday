...
class ActivityWatcher:
    ...
    def set_rules(self):
        ...
        if "always_active_pattern" in response:
            ...
            self.data["query"] = (
                self.data["query"][:3] + extra_codes + self.data["query"][3:]
            )
    ...
    def top_events(self, events_datas, event_type):
        def format_name(name, event_type):
            if event_type == "app":
                return name["app"]
            elif event_type == "cat":
        ...
        for item in events_datas[event_type + "_events"]:
            # 过滤掉需排除的事件title和app
            if event_type in ["title", "app"] and any(
                re.search(
                    pattern,
                    f'{item["data"].get("app","")} | {item["data"].get("title","")}',
                    re.I,
                )
            ...
            if item["duration"] > self.args["time"]["min_time_event"]:
                events.append(
                    f"- {format_name(item['data'], event_type)}: 耗时 {item['duration']/60:.1f} min"
                )
